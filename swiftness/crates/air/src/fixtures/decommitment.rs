...
use swiftness_commitment::table::{self, decommit::MONTGOMERY_R};
...
pub fn get() -> trace::Decommitment {
    ...
    let decommitment_values = FunVec::from_vec(vec![
        Felt::from_hex_unchecked(
    ...
    trace::Decommitment {
        original: table::types::Decommitment {
            values: decommitment_values,
            montgomery_values: FunVec::from_vec(
                decommitment_values.iter().map(|v| v * MONTGOMERY_R).collect(),
            ),
        },
